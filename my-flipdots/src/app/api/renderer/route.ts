...
import { WIDTH, HEIGHT, FPS} from "../../lib/settings"    
    ...
    export async function GET() {
        ...
        if (!cachedWeather || now - cachedAt > weather_TTL) {
            try{
                const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}/api/weather`);
                if (res.ok) {
                    cachedWeather = await res.json();
                    console.log("Fetched weather RAW", JSON.stringify(cachedWeather, null, 2));
                } else {
                    console.error("Weather API returned error:", res.status);
        ...
        if (cachedWeather && !cachedWeather.error && cachedWeather.main) {
            ...
            const cond = (
                ...
                cachedWeather.weather[0] && 
                cachedWeather.weather[0].main) || "";
